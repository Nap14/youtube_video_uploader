...
class LessonEditModal(ctk.CTkToplevel):
    def __init__(self, master, schedule, day=None, time=None, value=None, on_save=None, **kwargs):
        ...
        self.schedule, self.on_save = schedule, on_save
        self.old_day, self.old_time, self.value = day, time, value
        self.title("Edit Lesson" if day and time and value else "Add New Lesson")
        self.geometry("700x500"); self.transient(master); self.grab_set()
        self.grid_columnconfigure((0, 1), weight=1); self.create_widgets()

    def create_widgets(self):
        ctk.CTkLabel(self, text="Day *").grid(row=0, column=0, padx=20, pady=(15,0), sticky="w")
        ...
        ctk.CTkLabel(self, text="Time *").grid(row=0, column=1, padx=20, pady=(15,0), sticky="w")
        ...
        self.hour_cb.pack(side="left"); ctk.CTkLabel(tf, text=" : ").pack(side="left")
        self.min_cb = ctk.CTkComboBox(tf, values=["00", "15", "30", "45"], width=100)
        self.min_cb.pack(side="left")
        if self.old_time:
            h, m = self.old_time.split(':'); self.hour_cb.set(h)
            self.min_cb.set(m if m in ["00", "15", "30", "45"] else "00")
        ...
        self.img_entry = ctk.CTkEntry(img_f, width=500); self.img_entry.pack(side="left", fill="x", expand=True)
        if self.value and isinstance(self.value, dict): self.img_entry.insert(0, self.value.get("thumbnail") or "")
        ctk.CTkButton(img_f, text="Browse", width=80, command=self.browse_img).pack(side="left", padx=10)

        f_cd = ctk.CTkFrame(self, fg_color="transparent"); f_cd.grid(row=6, column=0, columnspan=2, padx=20, pady=10, sticky="w")
        ctk.CTkLabel(f_cd, text="Duration:").pack(side="left")
        self.dur_entry = ctk.CTkEntry(f_cd, width=70); self.dur_entry.pack(side="left", padx=10)
        ...
        self.color_preview = ctk.CTkLabel(f_cd, text="", width=30, height=20, corner_radius=4, fg_color="gray"); self.color_preview.pack(side="left", padx=10)
        ...
        bf = ctk.CTkFrame(self, fg_color="transparent"); bf.grid(row=8, column=0, columnspan=2, padx=20, pady=30, sticky="ew")
        ctk.CTkButton(bf, text="Save", command=self.save, width=120, fg_color="#27ae60").pack(side="left", padx=5)
        if self.old_day and self.old_time and self.value: ctk.CTkButton(bf, text="Delete", fg_color="#e74c3c", command=self.delete, width=120).pack(side="left", padx=5)
    ...
    def pick_color(self):
        ...
        if hex_c and self.winfo_exists(): self.chosen_color = hex_c; self.color_preview.configure(fg_color=hex_c)
    ...
    def save(self):
        ...
        new_data.update({"name": name, "lms_course_id": self.lms_id_entry.get(), "duration": int(self.dur_entry.get() if self.dur_entry.get().isdigit() else 60), "thumbnail": self.img_entry.get(), "color": self.chosen_color})
        if d not in self.schedule: self.schedule[d] = {}
        self.schedule[d][t] = new_data
        if self.old_day and self.old_time and (d != self.old_day or t != self.old_time): del self.schedule[self.old_day][self.old_time]
