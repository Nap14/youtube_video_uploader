...
def load_schedule(file: str, logging):
    ...
    if not os.path.exists(file):
         print("\n" + "!"*50)
         print(f"ERROR: Schedule file '{file}' not found!")
         print("Please create this file in JSON format.")
         print("You can find a structure example in README.md.")
         print("!"*50 + "\n")
         logging.error(f"Schedule file {file} not found. Restart the program after creating the file.")
...
def get_course_name_by_date(folder_date, schedule):
    """
    Determines the course based on the recording's date and time.
    Returns a dictionary with course data (name, thumbnail path) or None.
    """
